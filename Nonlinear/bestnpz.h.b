...
# if defined BENTHIC
      ...
      real(r8) ::avgD,avgDF,avgPS,avgPL,dw  
      real(r8) ::sumD,sumDF,sumPS,sumPL
# endif
#   ifdef ICE_BIO
      integer :: ibioBI
#   endif      
...
#endif
#if defined STATIONARY2
       real(r8), dimension(IminS:ImaxS,NTS2(ng)) :: Stat2
#endif
      ...
      real(r8), dimension(IminS:ImaxS,JminS:JmaxS) :: PARs
...
#if defined CLIM_ICE_1D	
	 ice_thick(i,j) = MAX(0_r8,tclm(3,3,N(ng),i1CI)) 
...
# ifdef CLIM_ICE_1D
	  ...
	  if (itL(i,j,nstp,iIceLog).gt.0.and.itL(i,j,nnew,iIceLog).lt.0)THEN !new ice at this timestep
...
!          cff1=IceNH4(i,j,nnew)
!          cff2=IceNH4(i,j,nstp)
	  
!	   IceNH4(i,j,nnew)=cff2
!	   IceNH4(i,j,nstp)=cff1
	   	   
#ifdef STATIONARY2           
	
     	
!	Stat2(i,1)=IceNH4(i,j,nstp) 
!	Stat2(i,2)=IceNH4(i,j,nnew)
#endif  
...
! Initialize the ice biology
	   ...
	   elseif (IceLog(i,j,nstp).gt.0.and.IceLog(i,j,nnew).gt.0)THEN
	      IcePhL(i,j,nstp) = IcePhL(i,j,nstp)     
              IceNO3(i,j,nstp) = IceNO3(i,j,nstp)      
              IceNH4(i,j,nstp) = IceNH4(i,j,nstp) 
...
!  converted from deg C m/s to E/m2/day
            PARs(i,j) =PARfrac(ng) * srflx(i,j) * rho0 * Cp * 0.394848_r8
	    
!        print*,'PARfrac=',PARfrac
       

#endif
               
!----------------------------------------------------------- 
!  George Blamey''s version after Morel 1988 (in Loukos 1997) 0.428 -0.01-0.572
!-----------------------------------------------------------
          cff10=grid(ng) % h(i,j)
...
!              cff1 = k_extV * ( z_w(i,j,k) - z_w(i,j,k+1) )
...
!              cff2=cff4 * ( z_w(i,j,k) - z_r(i,j,k+1) )
...
#else
!  Version from Sarah's old C code 
          DO k=N(ng),1,-1
            DO i=Istr,Iend
...
!Denman limitation 
!g        NOup =MAX(0_r8,(Bio(i,k,iPhS)/ccr)*Pmaxs*MIN(LightLim, NOLim, IronLim))
        NOup =(Bio(i,k,iPhS)/ccr)*Pmaxs*MIN(LightLim, NOLim, IronLim)
...
!g          NHup = MAX(0_r8,(Bio(i,k,iPhS)/ccr) * Pmaxs * MIN(LightLim, NHLim))
          NHup = (Bio(i,k,iPhS)/ccr) * Pmaxs * MIN(LightLim, NHLim)
#endif
...
!g              NOup=MAX(0,(Bio(i,k,iPhL)/ccrPhL)*Pmaxs*MAX(LightLim ,NOLim,IronLim))
          NOup=(Bio(i,k,iPhL)/ccrPhL)*Pmaxs*MIN(LightLim ,NOLim,IronLim)

#ifdef STATIONARY         
	     Stat3(i,k,1) =NOup
	     Stat3(i,k,2) =LightLim
	     Stat3(i,k,7) =NOLim
             Stat3(i,k,1) =IronLim
	     Stat3(i,k,2) =MIN(LightLim ,NOLim,IronLim)
...
!g          NHup = MAX(0_r8,(Bio(i,k,iPhL)/ccrPhL) * Pmaxs * MAX(LightLim ,NHLim))
          NHup = (Bio(i,k,iPhL)/ccrPhL) * Pmaxs * MAX(LightLim ,NHLim)
...
!     &		 )* cff2 * cff3 * dtdays
     ...
     &	fpPhSCop * (Bio(i,k,iPhS)**2) * cff2 * cff3 * dtdays*xi
		 
     
                    bflx(iPhL,iCop)=bflx(iPhL,iCop)+                 &
     &  fpPhLCop  * (Bio(i,k,iPhL)**2) * cff2 * cff3 * dtdays*xi
		    
                    bflx(iMZS,iCop)=bflx(iMZS,iCop)+                &
     &	fpMZSCop  * (Bio(i,k,iMZS)**2) * cff2 * cff3 * dtdays*xi
...
!     & * cff2 * cff3 *dtdays
     ...
     &         fpPhSNCa * (Bio(i,k,iPhS)**2) * cff2 * cff3 *dtdays*xi
             bflx(iPhL,iNCaS)=bflx(iPhL,iNCaS)+             &
     &         fpPhLNCa * (Bio(i,k,iPhL)**2) * cff2 * cff3 *dtdays*xi
             bflx(iMZS,iNCaS)=bflx(iMZS,iNCaS)+                  &
     &         fpMZSNCa * (Bio(i,k,iMZS)**2) * cff2 * cff3 *dtdays*xi
             bflx(iMZL,iNCaS)=bflx(iMZL,iNCaS)+                  &
     &         fpMZLNCa * (Bio(i,k,iMZL)**2) * cff2 * cff3 *dtdays*xi
...
!g       
!g
!g         DBioBI(i,iIcePhL) = DBioBI(i,iIcePhL)-Hz(i,j,N(ng))*(fpPhLNCa* &
!g     &              (BioBI(i,iIcePhL)/Hz(i,j,N(ng)))**2*cff2 * cff3 * dtdays)
!g
!g
!g#if defined BIOFLUX && defined BEST_NPZ		        
...
# endif 
     ...
     &      fpPhSEup * (Bio(i,k,iPhS)**2) * cff2 * cff3 * dtdays*xi
                    bflx(iPhL,iEupS)=bflx(iPhL,iEupS)+                   &
     &      fpPhLEup * (Bio(i,k,iPhL)**2) * cff2 * cff3 * dtdays*xi
                    bflx(iMZS,iEupS)=bflx(iMZS,iEupS)+                   &
     &      fpMZSEup * (Bio(i,k,iMZS)**2) * cff2 * cff3 * dtdays*xi
                  bflx(iMZL,iEupS)=bflx(iMZL,iEupS)+                     &
     &      fpMZLEup * (Bio(i,k,iMZL)**2) * cff2 * cff3 * dtdays*xi
                    bflx(iCop,iEupS)=bflx(iCop,iEupS)+                   &
     &      fpCopEup * (Bio(i,k,iCop)**2) * cff2 * cff3 * dtdays*xi
...
!g          DO i=Istr,Iend
!g            DBioBI(i,iIcePhL) =                                  &
!g     &       DBioBI(i,iIcePhL)-Hz(i,j,N(ng))*(fpPhLEup*          &
!g     &         (BioBI(i,iIcePhL)/Hz(i,j,N(ng)))**2*cff2 * cff3 * dtdays)
!g
!g
!g#if defined BIOFLUX && defined BEST_NPZ		        
...
#ifdef ICE_BIO
!g          DO i=Istr,Iend
!g	     DBioBI(i,iIcePhL) = DBioBI(i,iIcePhL)-Hz(i,j,N(ng))*(fpPhLEup* &
!g     &              (BioBI(i,iIcePhL)/Hz(i,j,N(ng)))**2*cff2 * cff3 * dtdays)
!g
!g
!g#if defined BIOFLUX && defined BEST_NPZ		        
...
!		   cff3=1.0_r8
     ...
     &	         gammaJel * cff1 * cff2 * cff3 * dtdays
                ...
                !---------------------
#ifdef PROD3		
!                Prod(i,k,iJel) = gammaJel * Prod(i,k,iJel) + DBio(i,k,iJel)
     ...
     & fpCopJel * Bio(i,k,iCop)**2*Bio(i,k,iJel)* cff2 * cff3 * dtdays*xi           
             bflx(iNCaS,iJel)=bflx(iNCaS,iJel)+      &
     & fpNCaJel * Bio(i,k,iNCaS)**2*Bio(i,k,iJel)* cff2 * cff3 * dtdays*xi
	     bflx(iEupS,iJel)=bflx(iEupS,iJel)+     &
     & fpEupJel * Bio(i,k,iEupS)**2*Bio(i,k,iJel)* cff2 * cff3 * dtdays*xi
             bflx(iNCaO,iJel)=bflx(iNCaO,iJel)+       &
     & fpNCaJel * Bio(i,k,iNCaO)**2*Bio(i,k,iJel)* cff2 * cff3 * dtdays*xi
	     bflx(iEupO,iJel)=bflx(iEupO,iJel)+      &
     & fpEupJel * Bio(i,k,iEupO)*Bio(i,k,iJel)* cff2 * cff3 * dtdays*xi  
             bflx(iJel,iDetF) = bflx(iJel,iDetF) +              &
     &     ( 1.0_r8-gammaJel)*cff1*cff2* cff3 * Bio(i,k,iJel)*dtdays*xi
                  END IF
#endif
...
#else
             DBio(i,k,iCop) = DBio(i,k,iCop) -                      &
     &                      TFEup*mpredCop*dtdays*Bio(i,k,iCop)**2
!#if def DIAPAUSE
!               IF ( downward ) THEN
	        DBio(i,k,iNCaS) = DBio(i,k,iNCaS) -                  &
...
!#endif
     ...
     &                      TFEup*mpredEup*dtdays*Bio(i,k,iEupS)**2
               DBio(i,k,iNCaO) = DBio(i,k,iNCaO) -                  &
     &                      TFEup*mpredNCa*dtdays*Bio(i,k,iNCaO)**2
               DBio(i,k,iEupO) = DBio(i,k,iEupO) -                  &
...
!#endif     
     ...
     &                   + TFEup*mpredNCa * Bio(i,k,iNCaO)**2             &
     ...
     &                     ) * dtdays 
                ...
                IF (i.eq.3.and.j.eq.3) THEN
                    bflx(iCop,iDetF)= bflx(iCop,iDetF)+                &
     &             TFEup*mpredCop*dtdays*xi*Bio(i,k,iCop)**2
     
                    bflx(iNcaS,iDetF)= bflx(iNCaS,iDetF)+              &
     &             TFEup*mpredNCa*dtdays*xi*Bio(i,k,iNCaS)**2   
                    
                    bflx(iEupS,iDetF)= bflx(iEupS,iDetF)+              &
     &             TFEup*mpredEup*dtdays*xi*Bio(i,k,iEupS)**2 
                    bflx(iNcaO,iDetF)= bflx(iNCaO,iDetF)+              &
     &             TFEup*mpredNCa*dtdays*xi*Bio(i,k,iNCaO)**2   
                    
                    bflx(iEupO,iDetF)= bflx(iEupO,iDetF)+              &
     &             TFEup*mpredEup*dtdays*xi*Bio(i,k,iEupO)**2            
                END IF
#endif
...
!     &          TempFuncMZS(i,k)*BasalMetMZL*dtdays*Bio(i,k,iMZS)*xi
...
#endif
#ifdef PROD3         
!g	  Prod(i,k,iJel) = Prod(i,k,iJel) -                             &
!g     &              TFJel*BasalMetJel*Bio(i,k,iJel)*dtdays
#endif
...
!--------------------
	...
	sumPL=0_r8
             ...
             do k = 1,N(ng)
	      sumD = sumD + Bio(i,k,iDet)
	      sumDF = sumDF + Bio(i,k,iDetF)
	      sumPS = sumPS + Bio(i,k,iPhS)
	      sumPL = sumPL + Bio(i,k,iPhL)
             end do
...
!         dw=3.0_r8 !assume bottom 3ms available
	 dw=1.0_r8 !assume only bottom layer available
...
!Potential food available from of water column sea floor

       
       cff5=(prefD*BioB(i,k,iBenDet)/(prefD*BioB(i,k,iBenDet)+LupD))   &
     &    *prefD*BioB(i,k,iBenDet)
...
!uptake of each food category
       ...
       cff7=(cff0*cff1*BioB(i,k,iBen)*Rup)/(cff6+KupP)
       cff8=(cff0*cff2*BioB(i,k,iBen)*Rup)/(cff6+KupP)
       cff9=(cff0*cff3*BioB(i,k,iBen)*Rup)/(cff6+KupP)
       cff10=(cff0*cff4*BioB(i,k,iBen)*Rup)/(cff6+KupP)
       cff11=(cff0*cff5*BioB(i,k,iBen)*Rup)/(cff5+KupD)
	...
	Prod2(i,iBenPrd)=Prod2(i,iBenPrd)+(cff1+cff2+cff3+cff4)*dtdays
#endif
...
! for further secondary production
         ...
         DBio(i,k,iNH4)= DBio(i,k,iNH4)+xi*dtdays*0.0_r8      &
     &                 *(cff1+cff2+cff3+cff4+cff5)/Hz(i,j,k)
     
         DBioB(i,k,iBenDet)= DBioB(i,k,iBenDet)+dtdays        &
     &                 *(cff1+cff2+cff3+cff4+cff5)*1.0_r8

#ifdef STATIONARY2           
!	Stat2(i,2)=(cff1+cff2+cff3+cff4+cff5)*dtdays
	 ...
	 !---------------
   	   ...
   	   cff4=Qres*(((1_r8-eexD)*cff1)+((1_r8-eexD)*cff2)+((1_r8-eex) &
     &	        *cff3)+((1_r8-eex)*cff4)+((1_r8-eexD)*cff5))
...
#ifdef STATIONARY2           
!	Stat2(i,3)=cff6*dtdays
!	Stat2(i,4)=xi*cff6/Hz(i,j,k)
...
!     	Stat2(i,5)=cff0*BenPred*dtdays*BioB(i,k,iBen)**2 
#endif  
...
#ifdef STATIONARY2
                if(k.eq.18) then
!g		  Stat2(i,8)=Stat2(i,8)+sinkOUT(i,k)
                    endif
#endif
	       END DO
            END DO
...
!g	   DBio(i,N(ng),iNH4) = DBio(i,N(ng),iNH4)+cff2/Hz(i,j,N(ng))
...
!g 	      DBio(i,N(ng),iNH4) = DBio(i,N(ng),iNH4)-cff2/Hz(i,j,N(ng))
	      ...
	      DBioBI(i,iIceNH4)=DBioBI(i,iIceNH4)+cff2/aidz
!g 	      DBio(i,N(ng),iNH4) = DBio(i,N(ng),iNO3)-cff2/Hz(i,j,N(ng))
	     endif
	    endif
...
!g               DBio(i,N(ng),iNH4) = DBio(i,N(ng),iNH4)          &
!g     &	             + BioBI(i,iIceNH4)/Hz(i,j,N(ng))
        ...
        END DO
         ...
         DO i=Istr,Iend
           if (IceLog(i,j,nstp).ge.0_r8 )THEN
              IcePhL(i,j,nnew) = IcePhL(i,j,nnew)  
              IceNO3(i,j,nnew) =  IceNO3(i,j,nnew)     
              IceNH4(i,j,nnew) =  IceNH4(i,j,nnew)    
           else
	    IcePhL(i,j,nnew) =0_r8 
              IceNO3(i,j,nnew) =  0_r8     
              IceNH4(i,j,nnew) =  0_r8 
...
#  endif    
     ...
     &                             Prod(i,k,iJel)
        ...
        END DO
          ...
          DO i=Istr,Iend
#ifdef BENTHIC	  
            pt2(i,j,nnew,iBenPrd) = pt2(i,j,nstp,iBenPrd)+Prod2(i,iBenPrd) 
#endif	       
