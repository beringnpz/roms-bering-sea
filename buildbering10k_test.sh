...
# on either cluster1 or beast.
#
# This script is intended for compiling the in-progress branch of the 
# code.
...
export       SCRATCH_DIR=${MY_PROJECT_DIR}/BuildTmp
...
if [ $? -ne 0 ]; then
  ...
  echo "  Compilation failed: see ${SCRATCH_DIR}/buildouterr.txt for details"
	...
	mv buildouterr.txt ${SCRATCH_DIR}/buildouterr.txt
  mv oceanM oceanM_sinkrisebug
	echo "  Success: oceanM_sinkrisebug created"
fi