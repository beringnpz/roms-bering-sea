...
!
      ...
      deepval(iNCaS) = eps
      deepval(iEupS) = 0.08_r8
      deepval(iNCaO) = eps
      deepval(iEupO) = 0.08_r8
      deepval(iDet) = eps
      do k=1,N(ng)
         do j=JstrR,JendR
            do i=IstrR,IendR
               t(i,j,k,1,iNO3) = 9.0_r8 !18.0_r8
               t(i,j,k,1,iNH4) =  2.0_r8
               t(i,j,k,1,iDet) =  eps
...
!
      ...
      do i=IstrR,IendR
        do j=JstrR,JendR
          do k=1,N(ng)
            var5 = var3 * ( biod(i,j,k) - var4 )
	     if ( biod(i,j,k) .gt. 500.0_r8 ) then 
            t(i,j,k,1,iNCaO) = var1 - var2 * atan( var5 )
	    t(i,j,k,1,iNCaS) = 0.0_r8
	    else
	    t(i,j,k,1,iNCaS) = var1 - var2 * atan( var5 )
	    t(i,j,k,1,iNCaO) = 0.0_r8
	    endif
          enddo
        enddo
      enddo
...
!test for Bering Sea - initialize in Jan 
	  ...
	  do i=IstrR,IendR
              ...
              t(i,j,k,1,iPhS) = 1_r8 !t(i,j,k,1,iPhS)*.01_r8
              t(i,j,k,1,iPhL) = 1_r8 !t(i,j,k,1,iPhL)*.01_r8
              t(i,j,k,1,iMZS) = 0.0_r8
              t(i,j,k,1,iMZL) = t(i,j,k,1,iMZL)*.0001_r8
              t(i,j,k,1,iCop) = t(i,j,k,1,iCop)*.0001_r8
...
!
      ...
      enddo
         ...
         do i=IstrR,IendR
           do j=JstrR,JendR
             do k=1,NBL(ng)
                 bt(i,j,k,1,iBen) = MAX(bt(i,j,k,1,iBen),eps)
		 bt(i,j,k,1,iBenDet) = MAX(bt(i,j,k,1,iBenDet),eps)
