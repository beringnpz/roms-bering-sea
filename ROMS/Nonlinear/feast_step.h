...
!NOTE the following nfeast signifies do the biology every nfeast time steps, i.e. every day.
...
! Do this the first time through only, syncs modulus of feast_time with
! whatever the starting time-moment of the model was.
...
! copy other needed values
  ...
  ROMS_zoop(i,j,1,6) = MAX(0.0,bt(i,j,1,nstp,iBen)) &
	                 & / ROMS_depth(i,j,1)  
...
!=======================================================================
! Replace fish variables to tracer arrays. IS HZ NEEDED?
!=======================================================================
...
do spn=1,ALL_LINKS
     ...
     END SELECT
        ...
        if (prev_len(spn).gt.0) then
			     CFstart  = (CFbase(spn) * (NNbase(spn) - Npromote(spn)) +       &
		              & CFbase(prev_len(spn)) * Nloss) / NNstart 
		       CAstart  = (CAbase(spn) * (NNbase(spn) - Npromote(spn)) +      &
		              & CAbase(prev_len(spn)) * Nloss) / NNstart 
				 else
				    CFstart  = CFbase(spn)
				    CAstart  = CAbase(spn)
				 end if	 
